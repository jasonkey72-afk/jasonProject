...
실제 브라우저를 띄워 웹 자동화가 정말 동작하는지 확인합니다. (총 106건, 두 엔진 합계)
...
python tests\test_newtab.py      :: 같은 검사를 selenium 으로 11건
...
Shadow DOM 을 쓰는 웹 컴포넌트(`<my-widget>`)와,
**클릭하면 새 탭이 열리는 링크·버튼**(즉시 / 3초 뒤)도 포함되어 있습니다.
`newtab.html` 은 새 탭에만 있는 화면이라, 탭을 제대로 따라가지 못하면 검사가 실패합니다.
